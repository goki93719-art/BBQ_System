...
test("finished product metadata and starter cleanup are present", async () => {
  ...
  assert.match(app, /CategoryCreateDrawer/);
  assert.match(app, /role="dialog" aria-modal="true" aria-labelledby="item-create-title"/);
  assert.match(app, /role="dialog" aria-modal="true" aria-labelledby="category-create-title"/);
  assert.match(app, /保存并上架/);
  assert.match(app, /保存并启用/);
  assert.match(app, /商品描述/);
  assert.match(app, /请检查并完善 \$\{invalidFields\.length\} 项商品信息/);
  assert.match(app, /请检查并完善 \$\{invalidFields\.length\} 项品类信息/);
  assert.match(app, /aria-invalid=\{Boolean\(fieldErrors\.name\)\}/);
  assert.doesNotMatch(app, /window\.prompt\("商品名称"\)/);
  assert.doesNotMatch(app, /window\.prompt\("品类名称"\)/);
  assert.doesNotMatch(app, /window\.prompt\("品类编码/);
  ...
  assert.match(styles, /\.admin-form-field\.has-error/);
  assert.match(styles, /\.admin-field-error/);
  assert.match(apiRoute, /INVALID_CATEGORY_TYPE/);
  assert.match(apiRoute, /INVALID_CATEGORY_SORT/);
